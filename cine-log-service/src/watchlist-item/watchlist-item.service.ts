...
export class WatchlistItemService {
  constructor(@InjectRepository(WatchlistItem) private wathclistItemRepository: Repository<WatchlistItem>, private movieService : MovieService,private statusService : StatusService){}
  ...
  async create(movieId:number, user: User) {
      ...
      let watchlistItem = {
       ...
       const createdWatchlistItem = this.wathclistItemRepository.create(watchlistItem);
      ...
      if(!status)
        throw new BadRequestException("Status doesnt exist");
      ...
      async remove(id: number) {
         const existing = await this.findOneById(id);
            if(!existing)
              ...
              this.wathclistItemRepository.remove(existing);
      }
}
